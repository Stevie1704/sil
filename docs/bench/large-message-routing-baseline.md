...
The reproducible pre-#74 baseline for the copied Message path, required by
[issue #61](https://github.com/Stevie1704/sil/issues/61). It measured the
legacy one-slot Arena and motivated the bounded multi-slot change. It is a
decision input, not a threshold: nothing in the kernel's functional behavior
depends on any number in this document, and no test in the suite asserts a
duration.
...
| `inline_encode` / `inline_decode` | the base64 representation on the step line | 1 per Message on an inline Channel, and per Message the historical burst pushed out of the one-slot Arena |
...
- **Process delivery** — one Process participant, in both directions
  (kernel→process and process→kernel), inline against the historical one-slot Arena,
...
### Process transport: the legacy one-slot Arena fallback is the sharp edge
...
### #74 result: two slots remove the measured fallback

The current `make bench` report declares `slots: 2` on each shared-memory row.
Its two-Message burst records no inline encode or decode in either direction.
On the same machine, the measured user time and peak process-tree RSS changed
as follows (three-repeat medians for the current run):

| Direction | legacy slots | user s | peak RSS MiB | current slots | user s | peak RSS MiB |
|---|---|---|---|---|---|---|
| kernel→process, burst 2 | 1 | 0.875 | 53.8 | 2 | 0.138 | 40.5 |
| process→kernel, burst 2 | 1 | 1.844 | 213.4 | 2 | 0.137 | 35.2 |

The timing is evidence, not a threshold; the zero inline-copy counts are the
deterministic proof that the declared Arena carries the whole measured burst.
...
| #74 Multi-slot Channel Arena | Direct answer to the #58 evidence without leases: declare two slots for the measured burst so both Messages take the Arena, retaining inline only as a bounded fallback. |
