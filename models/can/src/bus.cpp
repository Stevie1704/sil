...
unsigned frame_bits(std::uint32_t id, std::span<const std::uint8_t> data) {
  ...
  unsigned crc = 0;  // ISO 11898-1 CRC-15 shift register
  for (bool bit : bits) {
  ...
  append(crc, 15);
  // Stuffing covers SOF through the CRC sequence; stuff bits count towards
  // the next run, and five equal bits at the end still get one.
  ...
  return unsigned(bits.size()) + stuffed + 1 + 1 + 1 + 7;
}

void Bus::receive(unsigned terminal, std::span<const std::uint8_t> operations, Nanoseconds now) {
...
void Bus::schedule(unsigned sender, std::span<const std::uint8_t> operation, Nanoseconds now) {
  require(bitrate_.has_value(), "configure the CAN bitrate before transmitting");
